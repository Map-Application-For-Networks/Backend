const expertiseTag = require('../models/researchExpertiseTag.model');
...
const addTag = async (req, res) => {
    try {
        ...
        if (typeof tagName !== 'string' || !tagName.trim()) {
            return res.status(400).json({ message: "Invalid research expertise are tag name. It must be a non-empty string." });
        }
        ...
        if (existingTag) {
            return res.status(409).json({ message: "A research expertise tag with this name already exists." });
        }
...
const showTags = async (req, res) => {
    try {
        ...
        if (!tags || tags.length === 0) {
            return res.status(404).json({ message: 'No research expertise tags found' });
        }
...
const deleteTag = async (req, res) => {
    try {
        ...
        if (!tag) {
            return res.status(404).json({ message: "Research expertise tag not found." });
        }
        ...
        if (!markers || markers.length === 0) {
            return res.status(200).json({
                message: "Research expertise tag deleted successfully. No markers needed updates.",
                deletedTag: tag,
                updatedMarkers: []
            });
        }
        ...
        res.status(200).json({
            message: "Research expertise tag deleted successfully and markers updated.",
            deletedTag: tag,
            updatedMarkers
        });

    } catch (error) {
        console.error("Error deleting research expertise tag:", error);
        res.status(500).json({ message: "An error occurred while deleting the research expertise tag." });
    }
};
