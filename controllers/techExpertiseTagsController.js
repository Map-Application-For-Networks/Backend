const TechnologyTag = require('../models/techExpertiseTag.model');
...
const addTag = async (req, res) => {
    try {
        ...
        if (typeof tagName !== 'string' || !tagName.trim()) {
            return res.status(400).json({ message: "Invalid technical expertise tag name. It must be a non-empty string." });
        }
        ...
        if (existingTag) {
            return res.status(409).json({ message: "A technical expertise tag with this name already exists." });
        }
...
const showTags = async (req, res) => {
    try {
        ...
        if (!tags || tags.length === 0) {
            return res.status(404).json({ message: 'No technical expertise tags found' });
        }
...
const deleteTag = async (req, res) => {
    try {
        ...
        if (!tag) {
            return res.status(404).json({ message: "technical expertise tag not found." });
        }
        ...
        if (!markers || markers.length === 0) {
            return res.status(200).json({
                message: "Technical expertise tag deleted successfully. No markers needed updates.",
                deletedTag: tag,
                updatedMarkers: []
            });
        }
        ...
        res.status(200).json({
            message: "Technical expertise tag deleted successfully and markers updated.",
            deletedTag: tag,
            updatedMarkers
        });
